...
THREE.VREffectPost = function ( renderer, onError, onReady, composer, onFinished ) {
	...
	this.setSize = function ( width, height ) {
		...
		fbo.setSize( renderer.getSize().width, renderer.getSize().height );
		composer.setSize( renderer.getSize().width, renderer.getSize().height )
	...
	this.render = function ( scene, camera ) {
		...
		if ( vrHMD && isPresenting ) {
			...
			renderer.render( scene, cameraL, fbo );

			// render right eye
			renderer.setViewport( renderRectR.x, renderRectR.y, renderRectR.width, renderRectR.height );
			renderer.setScissor( renderRectR.x, renderRectR.y, renderRectR.width, renderRectR.height );
			renderer.render( scene, cameraR, fbo );

			renderer.setScissorTest( false );

			composer.reset();
			composer.setSource( fbo );
			onFinished();
			composer.toScreen();
